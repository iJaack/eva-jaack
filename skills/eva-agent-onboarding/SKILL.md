...
- Treat MCP discovery descriptions as permission boundaries too. If a client-visible description says a tool does not publish, broadcast, call direct REST writes, or prove storage durability, keep the result at the lower rung; if a write-adjacent description omits that boundary or conflicts with the docs, report `blocked: live allowlist drift` instead of using the broader interpretation.
